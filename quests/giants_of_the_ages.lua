...
SMODS.current_mod.calculate = function(self, context)
  ...
  if not ((next(SMODS.find_mod('Multiplayer')) or next(SMODS.find_mod('NanoMultiplayer'))) and MP.LOBBY.code) then
    ...
    if not G.GAME.giants_start then
      if (G.jokers and G.jokers.cards and G.jokers.cards[1] and pokermon.get_type(G.jokers.cards[1]) == "Earth") and (G.jokers.cards[#G.jokers.cards] and pokermon.get_type(G.jokers.cards[#G.jokers.cards]) == "Water") then
