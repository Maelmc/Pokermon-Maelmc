...
local cufant = {
  ...
  calculate = function(self, card, context)
    ...
    if context.individual and context.end_of_round and context.cardarea == G.hand and SMODS.has_enhancement(context.other_card, "m_poke_hazard") then
      if not context.blueprint and card.ability.extra.to_steel > 0 then
...
local copperajah = {
  ...
  poke_custom_prefix = "maelmc",
  pos = {x = 5, y = 5},
  config = {extra = {hazard_ratio = 10, to_steel = 3, reset_steel = 3}},
  loc_vars = function(self, info_queue, card)
    type_tooltip(self, info_queue, card)
    -- just to shorten function
    local abbr = card.ability.extra
    ...
    return {vars = {to_add, abbr.hazard_ratio, abbr.reset_steel}}
  end,
  rarity = "poke_safari",
  cost = 9,
  stage = "One",
...
local gmax_copperajah = {
  ...
  pos = {x = 5, y = 2},
  soul_pos = { x = 5, y = 5 },
  config = {extra = {hazard_ratio = 10, to_steel = 5, reset_steel = 5, Xmult_mod = 0.1}},
  loc_vars = function(self, info_queue, card)
    type_tooltip(self, info_queue, card)
    -- just to shorten function
    local abbr = card.ability.extra
    ...
    return {vars = {to_add, abbr.hazard_ratio, abbr.reset_steel, abbr.Xmult_mod, 1 + abbr.Xmult_mod * steel_count }}
  end,
  rarity = "poke_mega",
  cost = 12,
  stage = "Mega",
  ...
  calculate = function(self, card, context)
    ...
    if context.cardarea == G.jokers and context.scoring_hand then
      if context.joker_main then
        ...
        return {
          message = localize{type = 'variable', key = 'a_xmult', vars = {1 + card.ability.extra.Xmult_mod * steel_count}}, 
          colour = G.C.XMULT,
          Xmult_mod = 1 + card.ability.extra.Xmult_mod * steel_count
        }
      end
    end
  end,
