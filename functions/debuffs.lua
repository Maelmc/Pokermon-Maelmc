...
SMODS.current_mod.set_debuff = function(card)
   ...
   if (G.GAME and G.GAME.blind and G.GAME.blind.name == "bl_maelmc_hearthflame_mask" and not G.GAME.blind.disabled) and (pokermon.is_type(card,"Fire") or SMODS.has_enhancement(card,'m_mult')) then return true end
   if (G.GAME and G.GAME.blind and G.GAME.blind.name == "bl_maelmc_wellspring_mask" and not G.GAME.blind.disabled) and (pokermon.is_type(card,"Water") or SMODS.has_enhancement(card,'m_bonus')) then return true end
   if (G.GAME and G.GAME.blind and G.GAME.blind.name == "bl_maelmc_cornerstone_mask" and not G.GAME.blind.disabled) and (pokermon.is_type(card,"Earth") or SMODS.has_enhancement(card,'m_stone')) then return true end
   if (G.GAME and G.GAME.blind and G.GAME.blind.name == "bl_maelmc_teal_mask" and not G.GAME.blind.disabled) and (pokermon.is_type(card,"Grass") or SMODS.has_enhancement(card,'m_lucky')) then return true end
