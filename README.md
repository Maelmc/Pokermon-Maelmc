...
A family of Joker that encourages raising all hand levels. Mega Gardevoir generates 2 Black Hole for each time you Mega it, so the idea is to mega, unmega, mega again etc etc to scale Gardevoir itself very quickly as long as you don't require the Mult Gardevoir is giving.
...
Teal is an incredible econ joker as it can scale very quickly and absurd amounts of money.
Wellspring scales the chips of Bonus cards, and they also give even more chips when scored. It can result in very high amount of chips.
Hearthflame is basically The Idol, but it'll only look for Mult cards in your deck which means it's much easier to predict on which card it'll land each round. It's also self enabling as it destroys cards.
Cornerstone is more gimmick than the rest. It allows to play Pair, Two Pair, 3oak, Full House, 4oak and 5oak using Stone Cards, which can help for synergises. It also gives Mult based on the stone cards' chips to help scoring.
